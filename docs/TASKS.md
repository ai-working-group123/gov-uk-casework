...
- Has a single public method: `call(system_prompt:, user_prompt:, max_tokens: 4000)`
...
Form should POST to admin_case_type_configs_path with params:
...
Form POSTs to answer_admin_case_type_config_path(@case_type_config)
...
3. FOR EACH MARKDOWN SECTION (decision tree, state transitions, 
   ...
   - Below each section: [Regenerate this section] button 
     (POSTs to regenerate_admin_case_type_config_path with 
     section param)
...
Form PATCHes to admin_case_type_config_path. 
Use admin layout. Tailwind CSS.
